 ...
 * 
 * Le message envoyé par le logicile à l'orduino permet d'ouvrir ou pas la porte 
...
// Attente extinction led ou fermeture porte
#define ATTENTE 1500 
// Délais de fermeture automatique de la porte 
#define TIMEOUT 4000

// Temps de depart de comptage
unsigned long init_time;

// Tableau contentent l'ID
byte identifiant[4];

// stock les données reçu via le port série
String reception="";
...
void loop() {
  ...
  if(reception == "TRUE"){
    ...
    // Initialisation du temps de départ pour la fermeture automatique
    init_time = millis();
    
    // Attente déclanchement capteur
    while(digitalRead(capteur_ir) == true){
      // Si capteur déclanché ou le temps écoulé, on sort de la boucle 
      if(digitalRead(capteur_ir) == false && millis() - init_time < TIMEOUT){
        break;
        }
     ...
     // Attente fermeture de la porte
     while(digitalRead(capteur_ir) == false){
      // Si le capteur n'est plus déclanché, on sort de la boucle
      if(digitalRead(capteur_ir) == true){
        break;
        }
      }
         
      monServo.write(0); // positionnement du servomoteur
      delay(ATTENTE); // Attente   
    }
