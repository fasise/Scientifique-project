...
import Boite_dialogue.SaveVisitBadge as SVB

Couleur    	 	 = "blue" # couleur de la barre inférieur et superieur
TextBottom  	 = "Développer par Rachel Système.......... Année 2021"

F_icone 			= "codefabla/icones/icone.ico" # icone de la boite de dialogue	
F_Zone_Saisie 		= "codefabla/icones/Fond_Zone_De_Saisie.jpeg" # Arrière plan de la zone de saisie
F_Zone_Photo 		= "codefabla/icones/Fond_Zone_De_Photo.jpeg" # Arrière plan de la zone de photo
Photo_Personnel 	= "codefabla/icones/Image_personnel.png" # image par defaut de la personne
		
I_bouton_fermer	= "icones/Bouton_fermer.png" # Icone bouton enregistrer
I_bouton_ok	    = "icones/Bouton_ok.png" # Icone bouton inserer image
...
class F_Demarrer(tk.simpledialog.Dialog):
	"""
		...
		### Ce fichier est utilisé dans la methode "ListePortDispo" du
		fichier "fenetrePrincipal.py & NouvelleCarte.py"
	...
	# Méthode permettant la lecture des doennées
	...
	def Connection_Reception_Donnee(self):
		...
		try:
			arduino = serial.Serial(self.port,self.baud,timeout=0.001)
			run = True
		except:
			tk.messagebox.showwarning(parent=self,title="Attention",
		...
		if execute:
			try:
				...
				with open("{}".format("Connexion/Manager/ID_MANAGER.rs.rachel"),
					"rb") as fichier:
					mon_depickler		 = pic.Unpickler(fichier)
