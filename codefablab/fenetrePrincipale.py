...
# module permettant d'écrire les fichiers
import pickle as pic
import os

import Boite_dialogue.BoiteInfo as BI
import Boite_dialogue.supprimerPersonnel as SP
import sous_fenetres.NouvelleCarte as NC
import sous_fenetres.ModifierCarte as MC 
import sous_fenetres.Demarrer as DM
import sous_fenetres.SaveManager as SM
import sous_fenetres.InfosDuPersonnel as IP
import ConnexionSerie.PortDisponible as PD
import sous_fenetres.CheckManager as CM

# titre de la fenetre principale
F_titre = " Module RFID  (arduino)"
# chemin d'accèss de l'icone de la fenetre principale
F_icone = "codefablab/icones/icone.ico"
# Couleur barre d'outils et barre inferieur
Coleur = "blue"

TextBottom = "Equipe projet"
...
F_imageFond  = "codefablab/icones/Image_fond.png"
F_demarrer 	 = "codefablab/icones/Demarrer.png"
F_ListeCarte = "codefablab/icones/ListeID.png"
F_newCarte   = "codefablab/icones/NewID.png"
F_Aproposde  = "codefablab/icones/icone.ico"
F_ResetPort  = "codefablab/icones/reset_port.png"
F_quitter 	 = "codefablab/icones/Quitter.png"

# variable contenant le message 
PortDispo = "Port non disponible" 
...
class mainWindow:
	...
	def __init__(self):
		...
		"""
		try:
			with open('{}'.format("Connexion/Manager/MANAGER.rs.rachel"),'rb') as fichier:
			...
			with open('{}/{}'.format("Connexion/Manager","ID_MANAGER.rs.rachel"),'rb') as fichier:
				pass
	...
	def configureConnexion(self):
		...
		"""
		Lecture des 4 premiers caractères correspondant au port disponible
			exemple : "COM5 (Arduino Mega at COM5)"

			Available_port = "COM5" a la fin de la boucle for
		"""
		for x in getport:
			Available_port += x 
			cnt+=1
		...
		try:
			...
			with open("{}".format("Connexion/Port&AccessBaud" + ".rs.rachel"),
			"wb") as fichier:
	...
	# Methode pour Quitter et ferme la fenêtre 	
	def Quitter(self,master=None):
	...
	def BarOutils(self):
		...
		exitButton = tk.Button(toolbar,relief=tk.GROOVE,image=self.ImgQuitter,
			height= B_H,width=B_W,command=self.Quitter)
		ToolTip(exitButton, msg="Fermer la fenètre")
		exitButton.pack(side=tk.RIGHT,padx=5,pady=2)
		
		# Configuration du combox choix BAUDRATE
		# label affichant le message "baudRate"
		labelBaudrate = tk.Label(toolbar, text="baudRate", bg="white").pack(side=tk.LEFT,padx=10)
		...
		labelPort = tk.Label(toolbar, text="Port", bg="white").pack(side=tk.LEFT,padx=10)
		...
		ResetPort = tk.Button(toolbar,relief=tk.GROOVE,height=B_H,width=B_W,
			image=self.ImgResetPort,command=self.ListePortDispo)
	...
	# Utilise la Class port disponible
	def ListePortDispo(self,master=None):
		listOfPortValue =["select port"]
		com = PD.ListePortCom() # Variable de type  PortDispo
